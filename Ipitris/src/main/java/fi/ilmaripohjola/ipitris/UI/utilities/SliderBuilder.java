...
public class SliderBuilder {
     ...
     *
     * @param startingScreen StartingScreen on which to attach the sliders created.
     */
    public static void createSliders(StartingScreen startingScreen) {
        ...
        JSlider widthSlider = new JSlider(JSlider.HORIZONTAL, 3, 40, startingScreen.getConfiguration().getBoardWidth());
        JSlider heightSlider = new JSlider(JSlider.HORIZONTAL, 4, 40, startingScreen.getConfiguration().getBoardHeight());
        JSlider scaleSlider = new JSlider(JSlider.HORIZONTAL, 2, 80, startingScreen.getConfiguration().getScale());
        addSliderTags(widthSlider, heightSlider, scaleSlider);
        setUpSliders(startingScreen, widthSlider, heightSlider, scaleSlider);
        JPanel[] panels = startingScreen.getPanels();
    ...
    private static void setUpSliders(StartingScreen ss, JSlider width, JSlider height, JSlider scale) {
    ...
    private static void setTickSpacing(JSlider width, JSlider height, JSlider scale) {
    ...
    private static void addSliderTags(JSlider width, JSlider height, JSlider scale) {
        width.setName("width");
        height.setName("height");
        scale.setName("scale");
    }

    private static void setPaintBooleans(JSlider width, JSlider height, JSlider scale) {
     ...
     */
    private static void createLabelTable(JSlider width, JSlider height, JSlider scale) {
    ...
    private static void setLableTables(Hashtable lableTable, JSlider width, JSlider height, JSlider scale) {
        width.setLabelTable(lableTable);
        height.setLabelTable(lableTable);
        scale.setLabelTable(lableTable);
    }

    private static void connectSliders(StartingScreen ss, JSlider width, JSlider height, JSlider scale) {
    ...
    private static void setFonts(JLabel widthName, JLabel heightName, JLabel scaleName) {
